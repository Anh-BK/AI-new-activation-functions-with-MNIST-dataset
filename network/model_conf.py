...
class model_conf(object):
    ...
    def __init__( self):
        ...
        # Call CNN 
        #with tf.device('/cpu:0'), tf.variable_scope("cnn_01")as scope:
        #    self.cnn_ins_01 = cnn_bl_conf(self.input_layer_val, self.mode)
        #    [nBatch, nDim]  = self.cnn_ins_01.final_output.get_shape()

        # Call DNN 
        with tf.device('/cpu:0'), tf.variable_scope("dnn_01")as scope:
        ...
        ### loss function
        with tf.device('/cpu:0'), tf.variable_scope("loss") as scope:
        ...
        ### Calculate Accuracy  
        with tf.device('/cpu:0'), tf.name_scope("accuracy") as scope:
